...
class Core400sComponent : public PollingComponent, public uart::UARTDevice {
  public:
    ...
    void power_on(void);
    void power_off(void);
    void set_fan_manual_highest(void);
  ...
  protected:
    ...
    void defer_publish_text(const std::string &defer_id,
                          esphome::text_sensor::TextSensor *sensor,
                          const std::string &new_state);
                        
    void defer_publish_number(const std::string &defer_id,
    ...
    std::vector<uint8_t> cmd_set_fan_manual_highest= {0XA5, 0X22, 0XFF, 0X07, 0X00, 0XFF, 0X01, 0X60, 0XA2, 0X00, 0X00, 0X01, 0X04};
